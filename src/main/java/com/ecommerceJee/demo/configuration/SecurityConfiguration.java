...
//	}
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		antMatchers("/signin/**").permitAll().//http://localhost:8080/signin est accéssible à tout le monde
//		antMatchers("/modifierpassword").permitAll().
		antMatchers("/modifierpassword").permitAll().

		antMatchers("/article/admin/**").hasAuthority("ADMIN").
		antMatchers("/article/**").hasAnyAuthority("ADMIN", "CLIENT").
		

		//http://localhost:8080/login est accéssible /article/admin/
		//aux utilisateurs qui ont le role ADMIN ou bien CLIENT
		antMatchers("/panier/**").hasAnyAuthority("ADMIN","CLIENT").
